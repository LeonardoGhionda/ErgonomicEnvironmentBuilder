...
#pragma warning restore 0414
        ...
        private readonly HashSet<string> excludedExtensionsSet = new();
        ...
        private readonly List<FileBrowserItem> allItems = new(16);

        [SerializeField]
        private FileBrowserQuickLink quickLinkPrefab;
        private readonly List<FileBrowserQuickLink> allQuickLinks = new(8);
...
#pragma warning restore 0649
        ...
        private readonly List<FileSystemEntry> validFileEntries = new();
        private readonly List<int> selectedFileEntries = new(4);
        private readonly List<string> pendingFileEntrySelection = new();

        private readonly List<string> submittedFileEntryPaths = new(4);
        private readonly List<string> submittedFolderPaths = new(4); // Used to check if all destination folders have write access
        private readonly List<FileSystemEntry> submittedFileEntriesToOverwrite = new(4); // Existing files selected by the user in save mode
...
#pragma warning restore 0414
        ...
        private readonly List<string> filterLabels = new(4);
        ...
        private readonly List<string> pathsFollowed = new();
        ...
        private readonly List<string> timedOutDirectoryExistsRequests = new(2);
#endif
        ...
        private void RefreshDriveQuickLinks()
        {
            ...
            {
                Vector2 anchoredPos = new(0f, -quickLinksContainer.sizeDelta.y);
                for (int i = 0; i < customQuickLinks.Length; i++)
                {
        ...
        internal void OnContextMenuTriggered(Vector2 pointerPos)
        {
            ...
            Vector2 position;
            _ = RectTransformUtility.ScreenPointToLocalPointInRectangle(rectTransform, pointerPos, canvas.worldCamera, out position);
                    ...
                    for (int startIndex = 0, nextStartIndex = 0; startIndex < filenameInput.Length; startIndex = nextStartIndex)
                    {
                        ...
                        if (!VerifyFilename(filename))
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    FileSystemEntry fileEntry = new(filename, FileBrowserHelpers.GetFilename(filename), "", true);
                                ...
                                else if (m_pickerMode != PickMode.Folders && FileBrowserHelpers.FileExists(filename))
                                {
                                    ...
                                    FileSystemEntry fileEntry = new(filename, fullPathFilename, GetExtensionFromFilename(fullPathFilename, AllExtensionsHaveSingleSuffix), false);
                                    if (FileSystemEntryMatchesFilters(fileEntry, AllExtensionsHaveSingleSuffix))
                                    {
                                        submittedFileEntryPaths.Add(filename);
            ...
            if (!item.IsDirectory)
            {
                ...
                else if (!allExtensionsHaveSingleSuffix)
                {
                    for (int j = 0; j < excludedExtensions.Length; j++)
                    {
                        ...
                        {
                            _ = excludedExtensionsSet.Add(extension);
                            continue;
                        }
                    }
                }
        ...
        {
            _ = StartCoroutine(CreateNewFolderCoroutine());
        }
        ...
        private IEnumerator CreateNewFolderCoroutine()
        {
            ...
            renameItem.Show(string.Empty, m_skin.FileSelectedBackgroundColor, m_skin.FolderIcon, (folderName) =>
            {
                ...
                _ = FileBrowserHelpers.CreateFolderInDirectory(CurrentPath, folderName);
        ...
        public void RenameSelectedFile()
        {
            ...
            renameItem.Show(fileInfo.Name, m_skin.FileSelectedBackgroundColor, GetIconForFileEntry(fileInfo), (newName) =>
            {
                ...
                if (fileInfo.IsDirectory)
                    _ = FileBrowserHelpers.RenameDirectory(fileInfo.Path, newName);
                else
                    _ = FileBrowserHelpers.RenameFile(fileInfo.Path, newName);
            ...
            else
            {
                ...
                for (int i = 0, fileCount = 0; i < selectedFileEntries.Count; i++)
                {
                    ...
                    if (m_pickerMode != PickMode.Files || !selectedFile.IsDirectory)
                    {
                        ...
                        {
                            _ = multiSelectionFilenameBuilder.Append("\"").Append(selectedFile.Name).Append("\" ");

                            if (++fileCount >= FILENAME_INPUT_FIELD_MAX_FILE_COUNT)
                            {
                                _ = multiSelectionFilenameBuilder.Append("...");
                                break;
                            }
                        }
                    }
        ...
        private bool CheckDirectoryExistsWithTimeout(string path, int timeout = 750)
        {
            ...
            try
            {
                ...
                System.Threading.Tasks.Task task = new(() => directoryExists = Directory.Exists(path));
                task.Start();
        ...
        public static void SetExcludedExtensions(params string[] excludedExtensions)
        {
            ...
            if (excludedExtensions != null)
            {
                for (int i = 0; i < excludedExtensions.Length; i++)
                {
                    ...
                    _ = Instance.excludedExtensionsSet.Add(excludedExtensions[i]);
